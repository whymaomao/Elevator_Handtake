...
//定义三个内存池
#define SRAMIN  0 //内部内存池SRAM
#define SRAMEX 1 //外部内存池SDRAM
#define CCM     2 //CCM内存池（仅CPU可以访问）

#define MemoryBank 3   //定义支持的内存池数
...
#define MEM2_MAX_SIZE			1024*4 *1024		
...
struct _m_mallco_dev
{
	void (*init)(u8);					//初始化
	u8 (*perused)(u8);		  	    	//内存使用率
	u8 	*membase[MemoryBank];				//内存池 管理SRAMBANK个区域的内存
	u16 *memmap[MemoryBank]; 				//内存管理状态表
	u8  memrdy[MemoryBank]; 				//内存管理是否就绪
};
